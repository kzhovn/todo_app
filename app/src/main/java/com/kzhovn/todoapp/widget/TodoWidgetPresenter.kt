...
object TodoWidgetPresenter {
    fun toRows(tasks: List<Task>, subtaskCounts: Map<Long, Pair<Int, Int>> = emptyMap()): List<WidgetTaskRow> =
        tasks.map { WidgetTaskRow(it.id, it.title, it.isComplete, it.isStarred, it.isMaybe, subtaskCounts[it.id]?.takeIf { c -> c.second > 0 }) }
}
